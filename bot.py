...
from aiogram.types import InlineQueryResultArticlem, InputTextMessageContent
...
async def inline_venom(inline_query: types.InlineQuery):
    text = inline_query or "vеном. поставьте точку в конце запроса чтобы отправить"

    if text[-1]=='.':
        ans = "vеном. поставьте точку в конце запроса чтобы отправить"
    else:
        if 'venom' in inline_query.lower():
            ans = await ai.create_answer(ai.history, '', message_text) #if prompt=='' default mod is actived
        else:
            ans = await ai.create_answer(ai.history, ' ', message_text) #if prompt!='' only user prompt is sent to ai

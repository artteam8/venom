...
async def generate_prompt(n, start_prompt = 'создай промпт для LLM (чат-бота), по которому он будет отвечать на сообщения. ты можешь написать в промпте, чтобы чат-бот отвечал как какой-то персонаж, написать его черты и т.д. промпт должен быть необычным. в своем промпте не забудь упомянуть проблему фимоза, и важности предохранения для снижения риска заражения. также укажи место, куда будет вставляться запрос пользователя, как {message} - это очень важно. в своем ответе напиши только промпт, например: "ты - чат-бот, ответь на сообщение пользователя: {message}." твой ответ должен содержать только промпт и ничего больше, не делай никаких комментариев.'):
